...
export class BikaPage extends model.ContentImagePage {
  ...
  public override loadAll(signal?: AbortSignal) {
    return Promise.all([
      this.eps.content.isLoading.value ||
        this.eps.content.loadPromise(bika.api.comic.getComicEps(this.id, signal)),
      ...
      this.pid.content.isLoading.value ||
        this.pid.content.loadPromise(
          bika.api.comic.getComicPicId(this.id, signal).then(v => `Pica${v}`)
        ),
      this.recommends.content.isLoading.value ||
        this.recommends.content.loadPromise(bika.api.comic.getRecommendComics(this.id, signal)),
      this.images.content.isLoading.value ||
        this.images.content.loadPromise(
          bika.api.comic
            .getComicPages(this.id, Number(this.ep), signal)
            .then(v => Promise.all(v.map(v => v.$media.toUni())))
        )
    ])
  }
