...
export const getBikaApiHeaders = (pathname: string, method: string) => {
  ...
  const rawSignature =
    `${pathname}${requestTime}${bikaStore.nonce.value}${method}C69BAF41DA5ABD1FFEDC6D2FEA56B`.toLowerCase()
  ...
  if (!isEmpty(bikaStore.loginToken.value))
    headers.push(['authorization', bikaStore.loginToken.value])
  return headers
}