...
export namespace _bikaApiComic {
  ...
  export const favouriteComic = PromiseContent.fromAsyncFunction(
    (id: string, signal?: AbortSignal) =>
      bikaStore.api.value!.post<ResultActionData<'favourite' | 'un_favourite'>>(
        `/comics/${id}/favourite`,
        {},
        { signal }
      )
  ...
  export const getComicInfo = PromiseContent.fromAsyncFunction(
    async (id: string, signal?: AbortSignal) => {
      ...
      const data = await bikaStore.api.value!.get<{ comic: BikaType.comic.RawFullComic } | false>(
        `/comics/${id}`,
        { signal }
      )
  ...
  export const getComicPicId = PromiseContent.fromAsyncFunction(
    async (id: string, signal?: AbortSignal) => {
      if (picIdStore.has(id)) return picIdStore.get(id)!
      const result = await bikaStore.share.value!.get<{ shareId: number }>(
        `/pic/share/set/?c=${id}`,
        { signal }
      )
  ...
  export const getComicIdByPicId = PromiseContent.fromAsyncFunction(
    async (picId: string, signal?: AbortSignal) => {
      const result = await bikaStore.share.value!.get<{ cid: string }>(
        `/pic/share/get/?shareId=${picId}`,
        { signal }
      )
      const id = result.cid
      return id
  ...
  export const getComicByPicId = PromiseContent.fromAsyncFunction(
    async (picId: string, signal?: AbortSignal) => {
  ...
  export const getRecommendComics = PromiseContent.fromAsyncFunction(
    (id: string, signal?: AbortSignal) =>
      createStructFromResponse(
  ...
  export const getComicEps = PromiseContent.fromAsyncFunction(
    async (id: string, signal?: AbortSignal): Promise<uni.ep.Ep[]> => {
      const stream = bikaStream(
        async (page, signal) =>
          (
            await bikaStore.api.value!.get<{
              eps: BikaType.api.pica.RawStream<BikaType.comic.RawComicEp>
            }>(`/comics/${id}/eps?page=${page}`, { signal })
          ).eps
      )
      ...
      return eps.map(
        ep => new uni.ep.Ep({ $$plugin: pluginName, name: ep.title, index: String(ep.order) })
      )
    }
  )

  type Pages = BikaType.api.pica.RawStream<BikaType.comic.RawPage>
  const comicsPagesCache = new Map<string, Pages[]>()
  export const getComicPage = (id: string, index: number, page: number, signal?: AbortSignal) =>
    createClassFromResponseStream(
      bikaStore.api.value!.get<{ pages: Pages }>(
        `/comics/${id}/order/${index}/pages?page=${page}`,
        { signal }
      ),
      _bikaComic.Page,
      'pages'
  ...
  export const getComicPages = async (id: string, index: number, signal?: AbortSignal) => {
    ...
    const _pages = (async () => {
      ...
      otherPages.push(
        ...(await Promise.all(
          times(firstPage.pages - 1, i => getComicPage(id, index, i + 2, signal))
        ))
      )
      const pages = flatten(
  ...
  export const createComicEpPageStream = (comicId: string, epIndex: number) =>
    bikaStream((page, signal) => getComicPage(comicId, epIndex, page, signal))
}