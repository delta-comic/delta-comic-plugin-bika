...
export const createClassFromResponse = async <T extends Record<string, any[]>, TResult>(
  source: PromiseLike<T>,
  box: new (source: T[keyof T][number]) => TResult,
  key: keyof T
) => {
...
export const createStructFromResponse = async <T extends Record<string, any[]>, TResult>(
  source: PromiseLike<T>,
  box: (source: T[keyof T][number]) => TResult,
  key: keyof T
) => {
  const data = await source
  const s = data[key]
...
export const createClassFromResponseStream = async <
  T extends Record<string, bika.api.pica.RawStream<any>>,
  TResult
>(
  v: Promise<T>,
  box: new (data: T[keyof T]['docs'][number]) => TResult,
  key: keyof T = 'comics'
): Promise<bika.api.pica.RawStream<TResult>> => {
  const data = await v
  const s = data[key]
  s.docs = s.docs.map(v => new box(v))
  return s
}
export const createStructFromResponseStream = async <
  T extends Record<string, bika.api.pica.RawStream<any>>,
  TResult
>(
  v: Promise<T>,
  box: (data: T[keyof T]['docs'][number]) => TResult,
  key: keyof T = 'comics'
): Promise<bika.api.pica.RawStream<TResult>> => {
  const data = await v
  const s = data[key]
...
export const spiltUsers = (userString = '') =>
  userString
    ...
    .map(v => v.trim())
    .filter(Boolean)
const isCosplay = (categories: string[]) =>
  categories.includes('COSPLAY') || categories.includes('Cosplay')
const createAuthor = (comic: bika.comic.RawBaseComic) =>
  spiltUsers(comic.author).map<uni.item.Author>(v => ({
    label: v,
...
export const createFullToUniItem = (comic: bika.comic.RawFullComic) =>
  bika.comic.BikaItem.create({
    ...
    author: createAuthorList(
      ...
      comic._creator && {
        ...
        icon: {
          $$plugin: pluginName,
          type: 'default',
          pathname: comic._creator.avatar?.path ?? ''
        },
        actions: ['search_uploader'],
        subscribe: 'uploader',
        $$meta: { user: comic._creator },
        $$plugin: pluginName
      },
    ...
    categories: comic.categories
      .map(v => ({
        name: v,
        group: '分类',
        search: { keyword: v, sort: bika.sorts[0].value, source: 'keyword' }
      }))
      .concat(
        comic.tags.map(v => ({
          name: v,
    ...
    cover: { $$plugin: pluginName, type: 'default', pathname: comic.thumb.path },
    title: comic.title,
    id: comic._id,
...
export const createCommonToUniItem = (comic: bika.comic.RawCommonComic) =>
  bika.comic.BikaItem.create({
    $$meta: { comic },
    $$plugin: pluginName,
    author: createAuthorList(
...
export const createLessToUniItem = (comic: bika.comic.RawLessComic, mustSafe = false) =>
  bika.comic.BikaItem.create({
    $$meta: { comic },
    $$plugin: pluginName,
