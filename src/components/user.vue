...
const $props = defineProps<{ user: bika.user.User; isSmall?: boolean }>()

const exp = computed(() => $props.user.customUser.exp ?? 0)
const needExp = computed(() => {
  return 100 * $props.user.customUser.level * ($props.user.customUser.level + 1)
})
...
const bikaCharTranslateMap: Record<string, string> = {
  ...
  test: '测试人员',
  admin: '管理员',
  lunar_performer: '拜年祭成员',
  god_girl: '女神',
  reviewer: '评论家'
}
const userCharactersTranslator = (character: string) =>
  Object.hasOwn(bikaCharTranslateMap, character) ? bikaCharTranslateMap[character] : character
</script>
  ...
  <NThing class="relative w-full overflow-hidden bg-(--van-background-2)">
    ...
    <template #header>
      <div class="mt-2 -mb-2 flex items-center">
        {{ user.name }}
        <div
          class="mx-1 flex items-center text-(--nui-primary-color)"
          v-if="user.customUser.gender == 'm'"
        >
          <svg
        ...
        <div
          class="mx-1 flex items-center text-(--nui-primary-color)"
          v-else-if="user.customUser.gender == 'f'"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            xmlns:xlink="http://www.w3.org/1999/xlink"
        ...
        <div class="mx-1 flex items-center text-(--nui-primary-color)" v-else>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            xmlns:xlink="http://www.w3.org/1999/xlink"
            class="w-4"
        ...
        <span class="mr-1 text-xs font-normal text-(--nui-primary-color)"
          >Lv{{ user.customUser.level }}</span
        >
      </div>
    </template>
    <template #description>
      <div class="flex w-full flex-wrap pb-1 *:mt-1 *:mr-1">
        <VanTag type="primary">
          {{ user.customUser.title }}
        </VanTag>
        <VanTag plain type="primary" v-for="c of user.customUser.characters">
